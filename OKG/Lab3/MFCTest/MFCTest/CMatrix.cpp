       ...
       CMatrix::CMatrix() // конструктор по умолчанию
       {
        n_rows=1;
        n_cols=1; // матрица 1x1
        array=new double*[n_rows];
        for(int i=0;i<n_rows;i++) array[i]=new double[n_cols];
        for(int i=0;i<n_rows;i++)
          for(int j=0;j<n_cols;j++) array[i][j]=0; // инициализирует все элементы матрицы нулями
        }
       ...
       //---------------------------------------------------------------------------------
       double &CMatrix::operator()(int i,int j) // позволяет обращаться к элементам матрицы как matrix(i, j)
       // i - номер строки
       // j - номер столбца
       {
       ...
       //---------------------------------------------------------------------------------
       double &CMatrix::operator()(int i) // позволяет обращаться к элементам вектора как matrix(i)
       ...
       //---------------------------------------------------------------------------------
       CMatrix CMatrix::operator-() // возвращает матрицу, все элементы которой умножены на -1
       // Оператор -M
       {
       ...
       //---------------------------------------------------------------------------------
       CMatrix CMatrix::operator+(CMatrix& M) // складывает две матрицы
           // eсли размерности не совпадают, выводится сообщение об ошибке.
       // Оператор M1+M2
       {	
       ...
       //---------------------------------------------------------------------------------
       CMatrix CMatrix::operator-(CMatrix& M) // вычитает одну матрицу из другой
           // если размерности не совпадают, выводится сообщение об ошибке
       // Оператор M1-M2
       {	
       	int bb=(n_rows==M.rows())&&(n_cols==M.cols());
       	if(!bb)
       ...
       //---------------------------------------------------------------------------------
       CMatrix CMatrix::operator*(CMatrix& M) // умножает две матрицы
           // eсли число столбцов первой матрицы не равно числу строк второй, выводится сообщение об ошибке
       ...
       //---------------------------------------------------------------------------------
       CMatrix CMatrix::operator=(const CMatrix& M) // присваивает одной матрице значения другой
           // eсли размерности не совпадают, выводится сообщение об ошибке
