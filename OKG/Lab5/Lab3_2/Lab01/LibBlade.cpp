...
CMatrix CreateTranslate2D(double dx, double dy) // Формирует матрицу 3x3 для переноса (смещения) объекта.

// Эта матрица используется в однородных координатах. Чтобы перенести точку (x, y), ее представляют как вектор-столбец [x, y, 1]. 
// Результат - новый вектор [x+dx, y+dy, 1], соответствующий точке, смещенной на dx по оси X и dy по оси Y.
...
// Формирует матрицу 3x3 для поворота объекта вокруг начала координат (0, 0) на угол fi (в градусах). 
// Положительный fi - против часовой стрелки.
...
//  Вычисляет матрицу преобразования координат из мировой системы RS в оконную RW
{
	CMatrix M(3, 3);
...
// Настраивает контекст устройства (dc) так, чтобы можно было рисовать, используя мировые координаты, 
// а система сама преобразовывала их в пиксельные координаты окна
...
CBlade::CBlade()
// Инкапсулирует все данные и логику, связанные с фигурой лопасти: ее геометрию, положение, вращение и отрисовку
{
...
// Вычисляет новые координаты всех вершин лопасти для следующего кадра анимации
{
...
// Рисует текущее состояние лопасти в контекст устройства dc
{
...
{
	// Вызывает вспомогательную функцию для рисования треугольников, формирующих лопасти
