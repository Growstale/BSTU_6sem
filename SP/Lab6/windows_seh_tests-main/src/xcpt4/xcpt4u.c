...
t12:;
    ...
    // Когда исключение, которое было сгенерировано внутри фильтра, передается на обработку другому фильтру на более 
    // высоком уровне стека, система SEH может рассматривать это уже не как "вложенный вызов" в том же смысле, как если бы 
    // исключение возникло внутри блока __except или __finally. Исключение теперь "покинуло" контекст фильтра F3 и стало 
    // "обычным" исключением для фильтра F2.
    ...
    __try {
        Counter += 1;
        // Функции семейства Interlocked требуют, чтобы адрес, по которому они производят атомарную операцию, был выровнен в соответствии с типом данных, над которым они работают
