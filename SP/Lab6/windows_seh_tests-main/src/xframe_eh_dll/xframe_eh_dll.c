...
// »нициализаци€ глобальных указателей DLL данными из EXE
{
    sehD = fp;
...
void pass_through_2_dll(PASS_THROUGH_DESTINATION dest, int Disposition) {
    ...
    __except (EvalFilter_dll(Disposition)) // test_step 7 + return Disposition
    {
        TEST_STEP(29)
    }
}
...
void fault_and_catch_dll(void) {
    ...
    __except (EvalFilter_dll(EXCEPTION_EXECUTE_HANDLER)) // test_step 7 + return Disposition
    {
        TEST_STEP(37)
    }
}
...
void fault_nocatch_fin(void) {
    ...
    __except (EvalFilter_dll(EXCEPTION_CONTINUE_SEARCH)) // test_step 7 + return Disposition
    {
        TEST_STEP(41)
    }
}
