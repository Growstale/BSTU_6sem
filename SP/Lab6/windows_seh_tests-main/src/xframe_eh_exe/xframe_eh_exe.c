...
void catch_combinations(void) {
    ...
    __try
    {
        ...
        // в no_fault только STEP 43
        // gPassThrough это pass_through_dll из dll
    }
    __except (EvalFilter1(GetExceptionCode(), GetExceptionInformation()))
    {
        TEST_STEP(61)
    ...
    __try
    {
        ...
        */
        ...
        // EvalFilter2 это     TEST_STEP(5) return EXCEPTION_CONTINUE_SEARCH;

    }
    __except (EvalFilter1(GetExceptionCode(), GetExceptionInformation()))
    {
        // EvalFilter1 это    TEST_STEP(3) + return EXCEPTION_EXECUTE_HANDLER;

        TEST_STEP(61)
    }
    END_TEST(5 * 7 * 3 * 23 * 11 * 61)

    START_TEST // 3 as in the previous one 
    __try
    {
        (*gPassThrough)(raise_nocatch_fin);
        // gPassThrough это pass_through_dll из dll
    ...
    __try
    {
        ...
        // gPassThroughCatch это pass_through_catch_dll из dll
        /*
        void raise_nocatch_fin(void) {
            __try
            {
    ...
    {
        (*gFaultAndCatch)();
        // gFaultAndCatch это fault_and_catch_dll из dll
    }
    __except (EvalFilter1(GetExceptionCode(), GetExceptionInformation()))
    {
        TEST_STEP(61)
    ...
    __try
    {
        ...
        // gPassThroughCatch это pass_through_catch_dll из dll
        /* pass_through_catch_to_fault_nocatch_fin:
        
            (*gFaultNoCatchFin)();

        */
        // gFaultNoCatchFin это fault_nocatch_fin из dll
    }
    __except (EvalFilter1(GetExceptionCode(), GetExceptionInformation()))
    {
        TEST_STEP(61)
...
// Подготовка проблемной страницы памяти
{
    ...
    // Резервирует виртуальное адресное пространство, но не выделяет физическую память и не отображает ее
    // Любая попытка доступа к этому адресу вызовет ошибку нарушения доступа (Access Violation, AV)
...
void CommitFaultPage(void)
{
    ...
    (
        sehD,
        sizeof(int),
        MEM_COMMIT, // Тип операции: передать физическую память
        PAGE_READWRITE
    );
...
EvalFilter3 (int nExcept, LPEXCEPTION_POINTERS pExcept)
{
    ...
    if ((nExcept == STATUS_ACCESS_VIOLATION) &&
        ...
        // ExceptionInformation[1]: Виртуальный адрес, доступ к которому вызвал ошибку
    {
        CommitFaultPage();
