...

{
    ...
    PEXCEPTION_RECORD ExceptionRecord;
    printf("    start except3...");

    ExceptionRecord = ExceptionPointers->ExceptionRecord;
    if ((ExceptionRecord->ExceptionCode == STATUS_INTEGER_OVERFLOW) &&
        // проверить, установлен ли конкретный бит (EXCEPTION_NESTED_CALL) в поле ExceptionFlags
        ((ExceptionRecord->ExceptionFlags & EXCEPTION_NESTED_CALL) == 0)) {
        printf("   if in except3...");
        *Counter += 17;
        ...
        ((ExceptionRecord->ExceptionFlags & EXCEPTION_NESTED_CALL) != 0)) {
        printf("    not if in except3...");
        *Counter += 19;
        return EXCEPTION_CONTINUE_SEARCH;
    }

    printf("    v3 in except3...");
...

{
    ...
    __try {
        printf("    start try_in_except1...");
        *Counter += 5;
        RaiseStatus(STATUS_INTEGER_OVERFLOW);
        printf("    end try_in_except1...");

    } __except(except3(GetExceptionInformation(), Counter)) {
        printf("    start except_in_except1...");
        *Counter += 7;
    }
...
t12:;
    ...
    // Когда исключение (E2), которое было сгенерировано внутри фильтра (F3), передается на обработку другому фильтру (F2) на более высоком уровне стека, система SEH может рассматривать это уже не как "вложенный вызов" в том же смысле, как если бы исключение возникло внутри блока __except или __finally. Исключение E2 теперь "покинуло" контекст фильтра F3 и стало "обычным" исключением для фильтра F2.
    ...
    __try {
        __try {
            ...
            printf("    start except_v2...");
            Counter += 2;
        }

    } __except(EXCEPTION_EXECUTE_HANDLER) {
        printf("    start except_v1...");
        Counter += 3;
    }
...
outside:
...

    {
        ...
        // для управления состоянием и режимом работы математического сопроцессора
        _controlfp_s(&PreviousControlFp, 0, 0);
        // эта строка РАЗРЕШАЕТ генерацию SEH-исключения при переполнении FPU
