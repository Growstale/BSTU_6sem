        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                lResult = RegSetValueEx(hKey,
                                        ...
                                        (BYTE *) pszServiceName,
                                        dwSize);
                // Это значение говорит COM, что для активации сервера с этим AppID нужно обратиться к SCM и запустить службу с 
                // указанным именем
                hr = (lResult == ERROR_SUCCESS) ? S_OK : HRESULT_FROM_WIN32(lResult);
            }
...
{
    // Использует API SCM (OpenSCManager, CreateService) для регистрации EXE как службы Windows
