...
class CComServerTest : public IComTest, public IVAVTest {
public:
  // IUnknown

    ULONG __stdcall AddRef() { return InterlockedIncrement(&_cRef); }
  ...
  IFACEMETHODIMP QueryInterface(__in REFIID riid, __out void **ppv) {
        static const QITAB qit[] =
        {
            ...
            QITABENT(CComServerTest, IVAVTest),
            { 0 },
        };
    ...
    if (SUCCEEDED(hr)) {
      *ppv = nullptr;
      ...
      else if (__uuidof(IVAVTest) == riid) {
          *ppv = static_cast<IVAVTest*>(this);
          hr = S_OK;
      }
  ...
  IFACEMETHODIMP CalculateSumOfDigits(LONG lInputNumber, LONG* plSumOfDigits)
  {
      if (plSumOfDigits == nullptr)
      {
          return E_POINTER;
      }
      *plSumOfDigits = 0;

      if (lInputNumber < 100 || lInputNumber > 999)
      {
          return E_INVALIDARG;
      }

      LONG sum = 0;
      LONG tempNum = lInputNumber;
