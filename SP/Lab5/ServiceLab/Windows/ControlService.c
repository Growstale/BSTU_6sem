...
int main() {
    ...
    // Открытие /dev/null для stdin, stdout, stderr
    ...
    while (running) {
        ...
        CopyFiles();
        sleep(copy_interval * 60); // Интервал в секундах
    }
...
void CopyFiles() {
    ...
    while ((entry = readdir(dir))) {
        if (entry->d_type != DT_REG) {
            continue; // Пропускаем нерегулярные файлы
        }
