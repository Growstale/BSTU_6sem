...
spool "SIMPLE_MIGRATION_&filename..log"

-- Password file execution
@passworddefinition.sql

PROMPT Dropping Role ROLE_SIMPLE_MIGRATION ...
DROP ROLE ROLE_SIMPLE_MIGRATION ;
PROMPT Creating Role ROLE_SIMPLE_MIGRATION ...
CREATE ROLE ROLE_SIMPLE_MIGRATION ;

-- PROMPT Drop HR user
-- drop user HR cascade;
   
PROMPT Create user HR
CREATE USER HR IDENTIFIED BY &&HR_password PASSWORD EXPIRE ACCOUNT LOCK /* DEFAULT TABLESPACE USERS  TEMPORARY TABLESPACE TEMP */;
GRANT CREATE SESSION, RESOURCE, CREATE VIEW, CREATE MATERIALIZED VIEW, CREATE SYNONYM, UNLIMITED TABLESPACE TO HR;
...
GRANT ALL ON utils TO ROLE_SIMPLE_MIGRATION;
grant ALL on utils to HR with grant option;

set define on
prompt connecting to HR
...
CREATE TABLE Employee (
  ...
  Name VARCHAR2(90 CHAR) NOT NULL,
  BirthDate DATE NOT NULL,
  Address VARCHAR2(150 CHAR)
);


PROMPT Creating Primary Key Constraint PK__Employee__7AD04FF11CFB424A on table Employee ... 
ALTER TABLE Employee
ADD CONSTRAINT PK__Employee__7AD04FF11CFB424A PRIMARY KEY
(
  EmployeeID
)
ENABLE
...
GRANT ALL ON Employee TO ROLE_SIMPLE_MIGRATION;
-- DROP TABLE BusinessTrip CASCADE CONSTRAINTS;


PROMPT Creating Table BusinessTrip ...
CREATE TABLE BusinessTrip (
  BusinessTripID NUMBER(10,0) NOT NULL,
  Bonus NUMBER(10,0) NOT NULL,
  EmployeeID NUMBER(10,0) NOT NULL,
...
ALTER TABLE BusinessTrip MODIFY (Bonus DEFAULT (0));
ALTER TABLE BusinessTrip MODIFY (StartDate DEFAULT SYSDATE);
ALTER TABLE BusinessTrip MODIFY (EndDate DEFAULT SYSDATE);
PROMPT Creating Primary Key Constraint PK__Business__75EA534A182D1A02 on table BusinessTrip ... 
ALTER TABLE BusinessTrip
ADD CONSTRAINT PK__Business__75EA534A182D1A02 PRIMARY KEY
(
  BusinessTripID
)
ENABLE
...
GRANT ALL ON BusinessTrip TO ROLE_SIMPLE_MIGRATION;
-- DROP TABLE Education CASCADE CONSTRAINTS;


PROMPT Creating Table Education ...
...
ALTER TABLE Education MODIFY (Bonus DEFAULT (0));
ALTER TABLE Education MODIFY (EducationDate DEFAULT SYSDATE);
PROMPT Creating Primary Key Constraint PK__Educatio__4BBE38E5BD8F4D6F on table Education ... 
ALTER TABLE Education
ADD CONSTRAINT PK__Educatio__4BBE38E5BD8F4D6F PRIMARY KEY
(
  EducationID
)
ENABLE
...
GRANT ALL ON Education TO ROLE_SIMPLE_MIGRATION;
...
ALTER TABLE Position MODIFY (Salary DEFAULT (0));
PROMPT Creating Primary Key Constraint PK__Position__60BB9A59E5C11C27 on table Position ... 
ALTER TABLE Position
ADD CONSTRAINT PK__Position__60BB9A59E5C11C27 PRIMARY KEY
(
  PositionID
)
ENABLE
...
GRANT ALL ON Position TO ROLE_SIMPLE_MIGRATION;
...
PROMPT Creating Primary Key Constraint PK__Departme__B2079BCDCD3D553D on table Department ... 
ALTER TABLE Department
ADD CONSTRAINT PK__Departme__B2079BCDCD3D553D PRIMARY KEY
(
  DepartmentID
)
ENABLE
...
GRANT ALL ON Department TO ROLE_SIMPLE_MIGRATION;
-- DROP TABLE VacationType CASCADE CONSTRAINTS;


PROMPT Creating Table VacationType ...
CREATE TABLE VacationType (
  VacationTypeID NUMBER(10,0) NOT NULL,
  Name VARCHAR2(30 CHAR) NOT NULL,
  NumberOfDays NUMBER(10,0) NOT NULL
);


ALTER TABLE VacationType MODIFY (NumberOfDays DEFAULT (0));
PROMPT Creating Primary Key Constraint PK__Vacation__22E54656FACB6133 on table VacationType ... 
ALTER TABLE VacationType
ADD CONSTRAINT PK__Vacation__22E54656FACB6133 PRIMARY KEY
(
  VacationTypeID
)
ENABLE
...
GRANT ALL ON VacationType TO ROLE_SIMPLE_MIGRATION;
-- DROP TABLE Vacation CASCADE CONSTRAINTS;


PROMPT Creating Table Vacation ...
CREATE TABLE Vacation (
  VacationID NUMBER(10,0) NOT NULL,
  StartDate DATE NOT NULL,
  EmployeeID NUMBER(10,0) NOT NULL,
  VacationTypeID NUMBER(10,0) NOT NULL
);
...
ALTER TABLE Vacation MODIFY (StartDate DEFAULT SYSDATE);
PROMPT Creating Primary Key Constraint PK__Vacation__E420DF84A162665C on table Vacation ... 
ALTER TABLE Vacation
ADD CONSTRAINT PK__Vacation__E420DF84A162665C PRIMARY KEY
(
  VacationID
)
ENABLE
...
GRANT ALL ON Vacation TO ROLE_SIMPLE_MIGRATION;
-- DROP TABLE EmployeePosition CASCADE CONSTRAINTS;


PROMPT Creating Table EmployeePosition ...
CREATE TABLE EmployeePosition (
  ...
  PositionID NUMBER(10,0) NOT NULL,
  DepartmentID NUMBER(10,0) NOT NULL,
  PositionHierarchy RAW(8000)
);


ALTER TABLE EmployeePosition MODIFY (EmploymentDate DEFAULT SYSDATE);

GRANT ALL ON EmployeePosition TO ROLE_SIMPLE_MIGRATION;
PROMPT Creating Index idx_business_trip_employee_id on BusinessTrip ...
CREATE INDEX idx_business_trip_employee_id ON BusinessTrip
(
  EmployeeID
) 
;
PROMPT Creating Index idx_education_employee_id on Education ...
CREATE INDEX idx_education_employee_id ON Education
(
  EmployeeID
) 
;
PROMPT Creating Index idx_vacation_employee_id on Vacation ...
CREATE INDEX idx_vacation_employee_id ON Vacation
(
  EmployeeID
) 
;
PROMPT Creating Index idx_employee_position_employee_id on EmployeePosition ...
CREATE INDEX idx_employee_position_employee_id ON EmployeePosition
(
  EmployeeID
) 
;
...
alter session set current_schema=HR;
set define off
CREATE OR REPLACE FORCE VIEW EmployeeFullInfo 
AS 
   SELECT e.EmployeeID ,
          e.Name ,
          e.BirthDate ,
     ...
     FROM Employee e
            ...
            JOIN Department D   ON ep.DepartmentID = D.DepartmentID
            JOIN RANK r   ON ep.RankID = r.RankID;


GRANT ALL ON EmployeeFullInfo TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE FORCE VIEW EmployeeBusinessTrips 
AS 
   SELECT e.Name EmployeeName  ,
          ...
          bt.Bonus 
     FROM Employee e
            JOIN BusinessTrip bt   ON e.EmployeeID = bt.EmployeeID;


GRANT ALL ON EmployeeBusinessTrips TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE FORCE VIEW EmployeeEducationHistory 
AS 
   SELECT e.Name EmployeeName  ,
          e.EmployeeID ,
          ed.Name EducationName  ,
          ed.EducationDate ,
          ed.Bonus 
     FROM Employee e
            JOIN Education ed   ON e.EmployeeID = ed.EmployeeID;


GRANT ALL ON EmployeeEducationHistory TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE FORCE VIEW EmployeeVacationInfo 
AS 
   SELECT e.Name EmployeeName  ,
          e.EmployeeID ,
          v.StartDate ,
          vt.Name VacationType  ,
          vt.NumberOfDays 
     FROM Employee e
            JOIN Vacation v   ON e.EmployeeID = v.EmployeeID
            JOIN VacationType vt   ON v.VacationTypeID = vt.VacationTypeID;


GRANT ALL ON EmployeeVacationInfo TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE FUNCTION CalculateTotalBonus
(
  v_EmployeeID IN NUMBER
)
RETURN NUMBER
AS
   v_TotalBonus NUMBER(10,0);
...
BEGIN
   SELECT NVL(SUM(Bonus) , 0) 
     ...
     FROM ( SELECT Bonus 
            ...
            SELECT Bonus 
            FROM Education 
             WHERE  EmployeeID = v_EmployeeID ) Bonuses;
   RETURN v_TotalBonus;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON CalculateTotalBonus TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE PROCEDURE AddSubordinate
(
  v_SupervisorID IN NUMBER,
  v_EmployeeID IN NUMBER,
  v_Phone IN VARCHAR2,
  v_Email IN VARCHAR2,
  v_EmploymentDate IN DATE,
  v_PositionID IN NUMBER,
  v_DepartmentID IN NUMBER
)
AS
   -- Получаем иерархию для выбранного узла
   v_SupervisorNode UDT.HIERARCHYID%TYPE;
   -- Найдем следующий доступный подчиненный узел
   v_NewNode UDT.HIERARCHYID%TYPE;

BEGIN

   SELECT PositionHierarchy 

     INTO v_SupervisorNode
     FROM EmployeePosition 
    WHERE  EmployeeID = v_SupervisorID;
   SELECT MAX(GetDescendant(NULL, NULL))  

     INTO v_NewNode
     FROM EmployeePosition 
    WHERE  IsDescendantOf(v_SupervisorNode) = 1;
   -- Вставляем нового подчиненного
   INSERT INTO EmployeePosition
     ( EmployeeID, Phone, Email, EmploymentDate, PositionID, DepartmentID, PositionHierarchy )
     VALUES ( v_EmployeeID, v_Phone, v_Email, v_EmploymentDate, v_PositionID, v_DepartmentID, v_NewNode );

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON AddSubordinate TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE PROCEDURE GetEmployeeHierarchy
(
  v_EmployeeID IN NUMBER
)
AS
   -- Получаем иерархию сотрудника
   v_HierarchyID UDT.HIERARCHYID%TYPE;
   -- Получаем подчиненных сотрудников и отображаем иерархию
...
BEGIN
   ...
   OPEN  v_cursor FOR
      SELECT EP.EmployeeID ,
             E.Name ,
             ToString() HierarchyPath  ,
             GetLevel() HierarchyLevel  
        FROM EmployeePosition EP
               JOIN Employee E   ON EP.EmployeeID = E.EmployeeID
       WHERE  IsDescendantOf(v_HierarchyID) = 1
        ORDER BY EP.PositionHierarchy ;
      DBMS_SQL.RETURN_RESULT(v_cursor);

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON GetEmployeeHierarchy TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE PROCEDURE GetSubordinatesByEmployeeID
(
  v_EmployeeID IN NUMBER
)
AS
   -- Получаем иерархию текущего сотрудника
   v_EmployeeHierarchy UDT.HIERARCHYID%TYPE;
   -- Завершаем предыдущую инструкцию точкой с запятой
   v_cursor SYS_REFCURSOR;
...
BEGIN
   ...
   SELECT PositionHierarchy 
     ...
     INTO v_EmployeeHierarchy
     FROM EmployeePosition 
    WHERE  EmployeeID = v_EmployeeID;
   -- Проверяем, существует ли сотрудник
   IF v_EmployeeHierarchy IS NULL THEN
    
   BEGIN
      DBMS_OUTPUT.PUT_LINE('Сотрудник не найден или не имеет иерархии.');
      RETURN;
   
   END;
   END IF;
   OPEN  v_cursor FOR
      WITH EmployeeHierarchy AS ( SELECT ep.EmployeeID ,
                                         ep.PositionHierarchy ,
                                         GetLevel() LEVEL_  
     FROM EmployeePosition ep
    WHERE  IsDescendantOf(v_EmployeeHierarchy) = 1
   UNION ALL 
   SELECT ep.EmployeeID ,
          ep.PositionHierarchy ,
          GetLevel() LEVEL_  
     FROM EmployeePosition ep
            JOIN EmployeeHierarchy eh   ON GetAncestor(1) = eh.PositionHierarchy ) 
      SELECT EmployeeID ,
             LEVEL_ 
...
END;
/
GRANT ALL ON GetSubordinatesByEmployeeID TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE PROCEDURE MoveSubtree
(
  v_FromEmployeeID IN NUMBER,
  v_ToEmployeeID IN NUMBER
)
AS
   -- Получаем иерархии для исходного и целевого узлов
   v_FromNode UDT.HIERARCHYID%TYPE;
   v_ToNode UDT.HIERARCHYID%TYPE;
   -- Найдем следующий доступный узел для вставки
   v_NewParent UDT.HIERARCHYID%TYPE;
...
BEGIN
   ...
   SELECT PositionHierarchy 
     ...
     INTO v_FromNode
     FROM EmployeePosition 
    WHERE  EmployeeID = v_FromEmployeeID;
   SELECT PositionHierarchy 

     INTO v_ToNode
     FROM EmployeePosition 
    WHERE  EmployeeID = v_ToEmployeeID;
   v_NewParent := v_ToNode ;
   GetDescendant(NULL,
                 NULL) ;
   -- Обновляем иерархию для всех подчиненных узлов
   UPDATE EmployeePosition
      SET PositionHierarchy = GetReparentedValue(v_FromNode, v_NewParent)
    WHERE  IsDescendantOf(v_FromNode) = 1;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON MoveSubtree TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE PROCEDURE ShowSubordinates
(
  v_EmployeeID IN NUMBER
)
AS
   v_Node UDT.HIERARCHYID%TYPE;
   v_cursor SYS_REFCURSOR;
...
BEGIN
   ...
   OPEN  v_cursor FOR
      SELECT EmployeeID ,
             ...
             GetLevel() LEVEL_  
        FROM EmployeePosition 
       WHERE  IsDescendantOf(v_Node) = 1
        ORDER BY PositionHierarchy ;
      DBMS_SQL.RETURN_RESULT(v_cursor);

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON ShowSubordinates TO ROLE_SIMPLE_MIGRATION;
...
END;
/
GRANT ALL ON UpdateEmployeePosition TO ROLE_SIMPLE_MIGRATION;
CREATE OR REPLACE TRIGGER trg_check_enddate
...
BEGIN
   BEGIN
      SELECT 1 INTO v_temp
       ...
       WHERE EXISTS ( SELECT 1 
                        FROM DUAL 
                       WHERE  :NEW.EndDate <= :NEW.StartDate );
   EXCEPTION
      WHEN OTHERS THEN
         NULL;
   END;
   ...
   BEGIN
      MERGE INTO BusinessTrip 
      USING (SELECT BusinessTrip.ROWID row_id, utils.dateadd('DAY', 1, :NEW.StartDate) AS EndDate
      FROM BusinessTrip ,INSERTED  
       WHERE BusinessTrip.BusinessTripID = :NEW.BusinessTripID
        AND BusinessTrip.EndDate <= :NEW.StartDate) src
      ON ( BusinessTrip.ROWID = src.row_id )
      WHEN MATCHED THEN UPDATE SET EndDate = src.EndDate;
...
alter session set current_schema=HR;
set define off
PROMPT Creating Foreign Key Constraint FK__Vacation__Employ__4F7CD00D on table Employee...
ALTER TABLE Vacation
ADD CONSTRAINT FK__Vacation__Employ__4F7CD00D FOREIGN KEY
(
  EmployeeID
)
REFERENCES Employee
...
PROMPT Creating Foreign Key Constraint FK__Education__Emplo__412EB0B6 on table Employee...
ALTER TABLE Education
ADD CONSTRAINT FK__Education__Emplo__412EB0B6 FOREIGN KEY
(
  EmployeeID
)
REFERENCES Employee
...
PROMPT Creating Foreign Key Constraint FK__BusinessT__Emplo__3C69FB99 on table Employee...
ALTER TABLE BusinessTrip
ADD CONSTRAINT FK__BusinessT__Emplo__3C69FB99 FOREIGN KEY
(
  EmployeeID
)
REFERENCES Employee
...
PROMPT Creating Foreign Key Constraint FK__EmployeeP__Emplo__534D60F1 on table Employee...
ALTER TABLE EmployeePosition
ADD CONSTRAINT FK__EmployeeP__Emplo__534D60F1 FOREIGN KEY
(
  EmployeeID
)
REFERENCES Employee
...
PROMPT Creating Foreign Key Constraint FK__Vacation__Vacati__5070F446 on table VacationType...
ALTER TABLE Vacation
ADD CONSTRAINT FK__Vacation__Vacati__5070F446 FOREIGN KEY
(
  VacationTypeID
)
REFERENCES VacationType
(
  VacationTypeID
)
ENABLE
...
PROMPT Creating Foreign Key Constraint FK__EmployeeP__Depar__5535A963 on table Department...
ALTER TABLE EmployeePosition
ADD CONSTRAINT FK__EmployeeP__Depar__5535A963 FOREIGN KEY
(
  DepartmentID
)
...
PROMPT Creating Foreign Key Constraint FK__EmployeeP__Posit__5441852A on table Position...
ALTER TABLE EmployeePosition
ADD CONSTRAINT FK__EmployeeP__Posit__5441852A FOREIGN KEY
(
  PositionID
)
REFERENCES Position
...
ALTER VIEW EmployeeFullInfo COMPILE;

ALTER VIEW EmployeeBusinessTrips COMPILE;

ALTER VIEW EmployeeEducationHistory COMPILE;

ALTER VIEW EmployeeVacationInfo COMPILE;
