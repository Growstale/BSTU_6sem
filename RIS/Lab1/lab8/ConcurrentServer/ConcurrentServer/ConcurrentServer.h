...
enum TalkersCommand { START, STOP, EXIT, STATISTICS, WAIT, SHUTDOWN, GETCOMMAND, OPEN_ACCEPT, CLOSE_ACCEPT};
...
extern volatile LONG TimeoutClients;
...
extern map<int, SOCKET> portMap;
extern mutex portMapMutex;

extern map<int, SOCKET> broadcastPortMap;

int OpenPort(int port);
int ClosePort(int port);
DWORD WINAPI AcceptServerThread(LPVOID pParam);
