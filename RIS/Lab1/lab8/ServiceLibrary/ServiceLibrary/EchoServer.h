...
struct SETSINCRO    // ответ сервера на запрос синхронизации счётчика времени
{
    char cmd[5];    // всегда значение SINC
    int correction; // значение, которое необходимо прибавить к значению счётчика времени
};
